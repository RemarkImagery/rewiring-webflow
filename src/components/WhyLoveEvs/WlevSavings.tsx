...
export default function WlevSavings(props: WlevSavingsProps) {
  ...
  const stats = [
    { number: "~$1.50/L", label: "equivalent cost to charge at home (incl. RUCs)" },
    { number: "20", label: "moving parts vs 2,000 in a petrol car" },
    { number: "67.5%", label: "the 5-year cost of a petrol car (EECA)" },
  ];
    ...
    <div className={`wlev-sav-root-${uid}`}>
      <section className={`wlev-sav-section-${uid}`} style={{ background: bgColor }}>
        <div className={`wlev-sav-inner-${uid}`}>
          ...
          <p className={`wlev-sav-body-${uid}`}>{body}</p>
          ...
          <div className={`wlev-sav-details-${uid}`}>
            <div className={`wlev-sav-detail-${uid}`}>
              ...
              <div>
                <strong className={`wlev-sav-detail-title-${uid}`}>Cheaper energy</strong>
                <p className={`wlev-sav-detail-text-${uid}`}>Charging at home at off-peak rates costs about $1.50/L equivalent. Rooftop solar pushes this even lower.</p>
              </div>
            </div>
            <div className={`wlev-sav-detail-${uid}`}>
              <div className={`wlev-sav-detail-icon-${uid}`}>
              ...
              <div>
                <strong className={`wlev-sav-detail-title-${uid}`}>Cheaper to maintain</strong>
                <p className={`wlev-sav-detail-text-${uid}`}>No oil changes, no exhaust system, no clutch or spark plugs. Hundreds of dollars saved every year.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .wlev-sav-section-${uid} { padding: 60px 24px; }
        }

        @media (max-width: 480px) {
          .wlev-sav-section-${uid} { padding: 40px 16px; }
          .wlev-sav-card-${uid} { padding: 28px 20px; }
        }
      `}</style>
    </div>
  );
