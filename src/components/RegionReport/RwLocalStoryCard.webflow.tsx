...
export default declareComponent(RwLocalStoryCard, {
  ...
  options: { ssr: true },
  props: {
    photo: props.Image({ name: "Photo (CMS: Image)", group: "Content" }),
    name: props.Text({ name: "Name", defaultValue: "First Last", group: "Content" }),
    location: props.Text({ name: "Location (CMS: Location name)", defaultValue: "Tawa, Wellington", group: "Content" }),
    entitySize: props.Text({ name: "Entity size", defaultValue: "4 people, 206 m²", group: "Content" }),
    highlightQuote: props.Text({ name: "Highlight Quote (short)", defaultValue: "We don't have to worry about fuel and energy prices.", group: "Content" }),
    largeQuote: props.Text({ name: "Large Quote (Read more)", defaultValue: "We don't have to worry about fuel and energy prices and we've really appreciated having our own generation and battery backup when storms have caused blackouts.", group: "Content" }),
    technologies: props.Text({ name: "Technologies (comma separated — CMS multi-refs can't bind, fill manually)", defaultValue: "Battery, EV, Heat pump, Hot water heat pump, Induction cooking, Solar", group: "Content" }),
